...
CREATE FUNCTION public.sp_get_airlinecompanies_by_username(user_name text) RETURNS TABLE(id bigint, user_id bigint, country_id bigint, name text)
    LANGUAGE plpgsql
    AS $$
BEGIN
        return query
...
ALTER FUNCTION public.sp_get_airlinecompanies_by_username(user_name text) OWNER TO postgres;
...
BEGIN
    ...
    select f.id,f.airline_company_id,f.origin_country_id,f.destination_country_id,f.departure_time,f.landing_time,f.remaining_tickets from flights f inner join airline_companies ac on f.airline_company_id = ac.id inner join users u on ac.user_id = u.id
    inner join customers c on u.id = c.user_id where c.id =customerid;
